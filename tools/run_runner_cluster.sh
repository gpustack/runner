...
usage() {
    ...
    echo "# Example"
    echo "    - VLLM_LOGGING_LEVEL=DEBUG ${name} 192.168.50.12,192.168.50.13 cuda12.8-vllm0.10.0 /path/to/data vllm serve --port 8080 --model /path/to/model ..."
    echo "# Images"
...
postprocess() {
    ...
    if [[ "${DRUN}" == "true" ]]; then
        ...
        for host in "${HOSTS[@]:1}"; do
            ssh_exec "${host}" "docker rm -f ${CONTAINER_NAME} 2>/dev/null" 2>/dev/null || true
        done
    fi
}
