...
usage() {
    ...
    echo "    - volume:          Host directory to mount as /data in the container"
...
ENV_FILE="$(mktemp)"
env | grep -v -E '^(PATH|HOME|LANG|PWD|SHELL|LOG|XDG|SSH|LC|LS|_|USER|TERM|LESS|SHLVL|DBUS)' >"${ENV_FILE}"
