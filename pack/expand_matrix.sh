...
for RULE in $(echo "${RULES}" | jq -cr '.[]'); do
    ...
    BACKEND_VARIANT=""
    echo "export BACKEND_VERSION=\${${BACKEND_UPPER}_VERSION}" >>"${INPUT_TEMPDIR}/envs_shared"
    if [[ "${BACKEND}" == "cann" ]]; then
        echo "export BACKEND_VARIANT=\${${BACKEND_UPPER}_ARCHS}" >>"${INPUT_TEMPDIR}/envs_shared"
    fi
    # Extract args.
    ...
    if [[ "${INPUT_TARGET}" == "runtime" ]]; then
        ...
        TAG="${TAG_PREFIX}python\${PYTHON_VERSION}"
        echo "export TAG=${TAG}" >>"${INPUT_TEMPDIR}/envs_dedicated"
    ...
    for SERVICE in "${SERVICES[@]}"; do
        ...
        # Prepare environment variables for sourcing.
        cp -f "${INPUT_TEMPDIR}/envs_shared" "${INPUT_TEMPDIR}/envs_dedicated"
        echo "export SERVICE_VERSION=\${${SERVICE_UPPER}_VERSION}" >>"${INPUT_TEMPDIR}/envs_dedicated"
        TAG="${TAG_PREFIX}${SERVICE}\${${SERVICE_UPPER}_VERSION}"
        echo "export TAG=${TAG}" >>"${INPUT_TEMPDIR}/envs_dedicated"
        # Value from environment variable.
        source "${INPUT_TEMPDIR}/envs_dedicated" &&
            rm -f "${INPUT_TEMPDIR}/envs_dedicated"
